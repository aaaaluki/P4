...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train -v 5 -T 0.001 -N10 -m 16 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
	   ...
	   # - The name of the world model will be used by gmm_verify in the 'verify' command below.
        gmm_train -v 5 -T 0.001 -N10 -m 16 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
	   ...
	   #   * <code> gmm_verify ... | tee $w/verif_${FEAT}_${name_exp}.log </code>
        gmm_verify -d work/$FEAT -e $FEAT -D work/gmm/$FEAT -E gmm -w $world\
